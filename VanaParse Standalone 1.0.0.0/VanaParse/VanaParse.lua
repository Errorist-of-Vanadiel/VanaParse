...
local defaults={
    schema=11, visible=true, paused=false, view='dynamic', scope='alliance', sort='dps', period='session', enemy='all',
...
if tonumber(settings.schema or 1)<11 then
    local previous_schema=tonumber(settings.schema or 1)
    settings.schema=11
    settings.columns=Core.merge(settings.columns or {},defaults.columns); settings.pins=settings.pins or {}; settings.self_pin=settings.self_pin~=false
...
local function record_magic_cast(actor,spell_name,outcomes,is_pet,master_actor)
...
local function process_action(act)
    ...
    if category==4 and relevant_actor then record_magic_cast(actor,action_label,magic_outcomes,is_pet,master_actor) end
...
local function hud_overview(source,actors,now)
    -- Dynamic is intentionally capped at 18 visible columns. The core combat
    -- metrics always remain visible; optional damage parents compete for the
    -- remaining slots based on their observed contribution in this view.
    local total=0
    for _,a in ipairs(actors or {}) do total=total+combined_damage(a) end
    local headers={'#','Player','Tot Dmg%','Tot Dmg','DPS','Melee','Acc','WS Dmg','WS Dmg%','WS H/M','WS Acc','WS Avg','SC#','SC Dmg','SC Dmg%'}
    local aligns={'right','left','right','right','right','right','right','right','right','right','right','right','right','right','right'}
    ...
    local sums={ranged=0,magic=0,pet=0,mb=0,crit=0}
    for _,a in ipairs(actors or {}) do
        sums.ranged=sums.ranged+(enabled_filter('ranged') and math.abs(net(a.ranged,a.dheal_ranged)) or 0)
        sums.magic=sums.magic+(enabled_filter('magic') and math.abs((tonumber(a.magic) or 0)-(tonumber(a.dheal_magic) or 0)-(tonumber(a.dheal_enspell) or 0)) or 0)
    ...
    local ranged_allowed=settings.columns.ranged~=false and enabled_filter('ranged')
    local pet_allowed=settings.columns.pet~=false and enabled_filter('pet')
    candidate('ranged','Ranged',sums.ranged,ranged_allowed)
    candidate('magic','Magic',sums.magic,enabled_filter('magic'))
    candidate('pet','Pet Dmg',sums.pet,pet_allowed)
    candidate('mb','MB Dmg',sums.mb,enabled_filter('magic'))
    candidate('crit','Crit%',sums.crit,settings.columns.crits==true and enabled_filter('melee'))
    table.sort(candidates,function(a,b) if a.score==b.score then return a.label<b.label end return a.score>b.score end)
    local optional={}
    while #headers<18 and #candidates>0 do
        local c=table.remove(candidates,1); optional[#optional+1]=c.key; headers[#headers+1]=c.label; aligns[#aligns+1]='right'
    end
    ...
    for i,a in ipairs(actors or {}) do
        ...
        local row={
            i,hud_player_name(a),('%.1f%%'):format(share),n(dmg),dash(combined_dps(a,source,now),n),
            enabled_filter('melee') and n(melee) or '-',enabled_filter('melee') and accuracy_text(a,false) or '-',
            enabled_filter('ws') and n(ws) or '-',enabled_filter('ws') and share_text(ws,total) or '-',
            enabled_filter('ws') and ('%d/%d'):format(tonumber(a.ws_hits) or 0,tonumber(a.ws_misses) or 0) or '-',
            enabled_filter('ws') and accuracy_pair_text(a.ws_attempts,a.ws_hits) or '-',enabled_filter('ws') and dash(avg(a.ws_mm),n) or '-',
            enabled_filter('sc') and tostring(tonumber(a.skillchain_count) or 0) or '-',enabled_filter('sc') and n(sc) or '-',enabled_filter('sc') and share_text(sc,total) or '-',
        }
        for _,key in ipairs(optional) do
            if key=='ranged' then row[#row+1]=n(net(a.ranged,a.dheal_ranged))
            elseif key=='magic' then row[#row+1]=n((tonumber(a.magic) or 0)-(tonumber(a.dheal_magic) or 0)-(tonumber(a.dheal_enspell) or 0))
            elseif key=='pet' then row[#row+1]=n(pet_net_damage(a))
            elseif key=='mb' then row[#row+1]=n(net(a.mb_damage,a.dheal_mb))
            elseif key=='crit' then row[#row+1]=(tonumber(a.melee_hits) or 0)>0 and pct(a.melee_crit,a.melee_hits) or '-' end
        end
        rows[#rows+1]=row
    end
    return format_parse_dynamic(headers,rows,aligns,{gap=1,min_widths={1,6,7,7,4,5,5}})
end
...
local function hud_compact(source,actors,now)
    ...
    for _,a in ipairs(actors or {}) do
        local total=math.abs(combined_damage(a)); if total>0 then
            local magic=enabled_filter('magic') and math.abs((tonumber(a.magic) or 0)-(tonumber(a.dheal_magic) or 0)-(tonumber(a.dheal_enspell) or 0)) or 0
            local pet=enabled_filter('pet') and math.abs(pet_net_damage(a)) or 0
            if enabled_filter('magic') and magic/total>=magic_threshold then show_magic=true end
            if enabled_filter('pet') and pet/total>=pet_threshold then show_pet=true end
        end
    end
    ...
    local rows={}
    for i,a in ipairs(actors or {}) do
        local row={tostring(i),hud_player_name(a),n(combined_damage(a)),dash(combined_dps(a,source,now),n),enabled_filter('melee') and accuracy_text(a,false) or '-',enabled_filter('ws') and ('%d/%d'):format(tonumber(a.ws_hits) or 0,tonumber(a.ws_misses) or 0) or '-',enabled_filter('ws') and dash(avg(a.ws_mm),n) or '-',enabled_filter('sc') and n(net(a.skillchain,a.dheal_skillchain)) or '-'}
        if show_magic then row[#row+1]=n((tonumber(a.magic) or 0)-(tonumber(a.dheal_magic) or 0)-(tonumber(a.dheal_enspell) or 0)) end
        if show_pet then row[#row+1]=n(pet_net_damage(a)) end
        rows[#rows+1]=row
    end
...
local function hud_max(source,actors,now)
    ...
    if section_on('healing') then section({'Cured','HPS','Cure#','Cleanse','Dispel','Aspir','CureLow','CureAvg','CurePk','MP','HP/MP'},{9,8,6,7,7,8,8,8,8,8,8},function(a) return {a.healing>0 and n(a.healing) or '-',dash(hps(a,source,now),n),a.cures>0 and a.cures or '-',a.cleanses>0 and a.cleanses or '-',a.dispels>0 and a.dispels or '-',a.aspir_recovery>0 and n(a.aspir_recovery) or '-',dash(a.cure_mm.low,n),dash(avg(a.cure_mm),n),dash(a.cure_mm.peak,n),a.mp_spent>0 and n(a.mp_spent) or '-',a.mp_spent>0 and ('%.1f'):format(a.healing/a.mp_spent) or '-'} end) end
    if section_on('recovery') then section({'Cure Rcvd','Self Cure'},{9,9},function(a) return {a.received>0 and n(a.received) or '-',a.self_healing>0 and n(a.self_healing) or '-'} end) end
...
local REPORT_NAME_COLORS={{96,176,255},{96,255,160},{255,215,96},{220,140,255},{255,150,120},{120,230,230},{200,220,120},{255,120,190}}
local function report_name(name)
    name=tostring(name or '-')
    local hash=0; for i=1,#name do hash=(hash*33+name:byte(i))%2147483647 end
    local c=REPORT_NAME_COLORS[(hash%#REPORT_NAME_COLORS)+1]
    return Core.color_text(name,c[1],c[2],c[3])
end

local function enemy_bucket_for(actor,name)
    local wanted=Core.lower(name)
    for enemy_name,bucket in pairs(actor and actor.enemy or {}) do
        if Core.lower(enemy_name)==wanted then return bucket,enemy_name end
    end
    return nil,nil
end

local function print_damage_report(source)
    local actors=display_actors(source)
    for _,a in ipairs(actors) do
        local melee=net(a.melee,a.dheal_melee); local ranged=net(a.ranged,a.dheal_ranged); local ws=net(a.ws_damage,a.dheal_ws)
        local sc=net(a.skillchain,a.dheal_skillchain); local magic=(tonumber(a.magic) or 0)-(tonumber(a.dheal_magic) or 0)-(tonumber(a.dheal_enspell) or 0)
        local other=net(a.other,a.dheal_other); local pet=pet_net_damage(a); local total=combined_damage(a)
        local sum=(enabled_filter('melee') and melee or 0)+(enabled_filter('ranged') and ranged or 0)+(enabled_filter('ws') and ws or 0)+(enabled_filter('sc') and sc or 0)+(enabled_filter('magic') and magic or 0)+(enabled_filter('other') and other or 0)+pet
        chat(207,('%s | Melee %s | Ranged %s | WS %s | SC %s | Magic %s | Enspell %s | Other %s | Pet %s | Tot %s | Difference %s'):format(report_name(a.name),n(melee),n(ranged),n(ws),n(sc),n(magic),n(a.enspell or 0),n(other),n(pet),n(total),n(total-sum)))
    end
end

local function print_magic_report(source,spell_name)
    if not source then chat(167,'No report data available.'); return end
    local actors=display_actors(source)
    local wanted=spell_name and Core.lower(spell_name) or nil
    chat(207,('Magic Report | %s%s'):format(settings.period,wanted and (' | '..spell_name) or ''))
    for _,a in ipairs(actors) do
        if wanted then
            local found_name,sp=nil,nil
            for name,data in pairs(a.spells or {}) do if Core.lower(name)==wanted then found_name=name; sp=data; break end end
            if sp then
                chat(207,('%s | %s | Cast %d | Tgt %d | Land %d | Res %d | NoEff %d | Dmg %s | Low %s | Avg %s | Peak %s | MB %d Dmg %s Avg %s Peak %s | NMB %d Dmg %s Avg %s Peak %s | Heal %s'):format(
                    report_name(a.name),found_name,sp.casts or 0,sp.targets or 0,sp.lands or 0,sp.resists or 0,sp.no_effect or 0,n(sp.damage or 0),dash(sp.damage_mm.low,n),dash(avg(sp.damage_mm),n),dash(sp.damage_mm.peak,n),sp.mb_hits or 0,n(sp.mb_damage or 0),dash(avg(sp.mb_mm),n),dash(sp.mb_mm.peak,n),sp.nonmb_hits or 0,n(sp.nonmb_damage or 0),dash(avg(sp.nonmb_mm),n),dash(sp.nonmb_mm.peak,n),n(sp.healing or 0)))
            else chat(207,report_name(a.name)..' | No spell data for '..spell_name) end
        else
            chat(207,('%s | Cast %d | Tgt %d | Land %d | Res %d | NoEff %d | Dmg %s | Low %s | Avg %s | Peak %s | MB %d/%s | NMB %d/%s'):format(
                report_name(a.name),a.magic_casts or 0,a.magic_targets or 0,a.magic_lands or 0,a.magic_resists or 0,a.magic_no_effect or 0,n(a.magic_damage or 0),dash(a.magic_mm.low,n),dash(avg(a.magic_mm),n),dash(a.magic_mm.peak,n),a.mb_count or 0,n(a.mb_damage or 0),a.nonmb_count or 0,n(a.nonmb_damage or 0)))
        end
    end
end

local function sorted_map_text(map,arrow)
    local rows={}; for name,value in pairs(map or {}) do rows[#rows+1]={name=name,value=tonumber(value) or 0} end
    table.sort(rows,function(a,b) return a.value>b.value end)
    local out={}; for _,r in ipairs(rows) do out[#out+1]=(arrow or '')..r.name..' '..n(r.value) end
    return #out>0 and table.concat(out,', ') or '-'
end

local function print_healing_report(source)
    if not source then chat(167,'No report data available.'); return end
    chat(207,'Cure Report | '..settings.period..' | Cured = HP restored; Cure R\'cvd = HP received')
    for _,a in ipairs(display_actors(source)) do
        chat(207,('%s | Cured %s -> %s | Cure R\'cvd %s <- %s | Self %s | Status Removal %d'):format(report_name(a.name),n(a.healing),sorted_map_text(a.healed_targets,''),n(a.received),sorted_map_text(a.healed_by,''),n(a.self_healing),tonumber(a.cleanses) or 0))
    end
end

local function print_dheal_report(source)
    if not source then chat(167,'No report data available.'); return end
    chat(207,'Damage Heal Report | '..settings.period)
    for _,a in ipairs(display_actors(source)) do local p=a.pet or {}
        chat(207,('%s | Total %s | Melee %s | Ranged %s | WS %s | SC %s | Magic %s | Pet %s'):format(
            report_name(a.name),n(total_dheal(a)),n(a.dheal_melee),n(a.dheal_ranged),n(a.dheal_ws),n(a.dheal_skillchain),n(a.dheal_magic),n(p.dheal or 0)))
    end
end

local function print_category_report(source,category)
    if not source then chat(167,'No report data available.'); return end
    category=Core.lower(category)
    local label=({melee='Melee',ranged='Ranged',sc='SC',pet='Pet',other='Other'})[category] or category
    local actors=display_actors(source); local total=0; local values={}
    for i,a in ipairs(actors) do
        local value=0
        if category=='melee' then value=net(a.melee,a.dheal_melee)
        elseif category=='ranged' then value=net(a.ranged,a.dheal_ranged)
        elseif category=='sc' then value=net(a.skillchain,a.dheal_skillchain)
        elseif category=='pet' then value=pet_net_damage(a)
        elseif category=='other' then value=net(a.other,a.dheal_other) end
        values[i]=value; total=total+value
    end
    chat(207,label..' Report | '..settings.period)
    for i,a in ipairs(actors) do
        local value=values[i] or 0; local share=total~=0 and (100*value/total) or 0
        chat(207,('%s | %s %s | Share %.1f%%'):format(report_name(a.name),label,n(value),share))
    end
end

local function print_ws_report(source)
    if not source then chat(167,'No report data available.'); return end
    chat(207,'WS Report | '..settings.period..' | H/M = landed uses / fully missed uses')
    for _,a in ipairs(display_actors(source)) do
        chat(207,('%s | WS Dmg %s | H/M %d/%d | Acc %s | Avg %s | Low %s | Peak %s'):format(
            report_name(a.name),n(net(a.ws_damage,a.dheal_ws)),tonumber(a.ws_hits) or 0,tonumber(a.ws_misses) or 0,
            pct(a.ws_hits,a.ws_attempts),dash(avg(a.ws_mm),n),dash(a.ws_mm.low,n),dash(a.ws_mm.peak,n)))
    end
end

local function print_report(source,enemy_override)
    if not source then chat(167,'No report data available.'); return end
    local now=Core.now(); local actors=display_actors(source); local elapsed,active=elapsed_active(source,now)
    local enemy=enemy_override or settings.enemy or 'all'
    if Core.lower(enemy)~='all' then
        chat(207,('Enemy Report | %s | %s | Elapsed %s | Active %s'):format(enemy,settings.period,Core.duration(elapsed),Core.duration(active)))
        for _,a in ipairs(actors) do
            local b=enemy_bucket_for(a,enemy)
            if b then
                chat(207,('%s | Dmg %s | Melee %s | WS %s (%d/%d miss %d avg %s low %s peak %s) | Ranged %s | Magic %s | SC %s | Enspell %s'):format(
                    report_name(a.name),n(b.damage),n(b.melee),n(b.ws),b.ws_hits or 0,b.ws_attempts or 0,b.ws_misses or 0,dash(avg(b.ws_mm),n),dash(b.ws_mm.low,n),dash(b.ws_mm.peak,n),n(b.ranged),n(b.magic),n(b.skillchain or 0),n(b.enspell or 0)))
            else
                chat(207,report_name(a.name)..' | No outgoing data for '..enemy)
            end
        end
        chat(207,'Enemy report currently filters enemy-linked outgoing damage. Healing/recovery remain encounter-context metrics in normal HUD views.')
        return
    end
    chat(207,('Report | %s | Elapsed %s | Active %s'):format(settings.period,Core.duration(elapsed),Core.duration(active)))
    for _,a in ipairs(actors) do
        chat(207,('%s | Tot Dmg %s | DPS %s | WS Dmg %s | WS H/M %d/%d | WS Acc %s | WSAvg %s | Low %s | Peak %s | Taken %s | Cured %s | Cure Rcvd %s'):format(report_name(a.name),n(combined_damage(a)),dash(combined_dps(a,source,now),n),n(net(a.ws_damage,a.dheal_ws)),a.ws_hits,a.ws_misses,pct(a.ws_hits,a.ws_attempts),dash(avg(a.ws_mm),n),dash(a.ws_mm.low,n),dash(a.ws_mm.peak,n),n(a.taken),n(a.healing),n(a.received)))
    end
end
...
Report.SCOPES={
    full='full',dps='dps',damage='dps',dmg='dps',
    magic='magic',spell='magic',
    cure='cure',cured='cure',healing='cure',
    defense='defense',def='defense',
    percent='percent',percentage='percent',['%']='percent',
    ws='ws',weaponskill='ws',melee='melee',ranged='ranged',range='ranged',
    sc='sc',skillchain='sc',pet='pet',
}

function Report.report_scope_label()
    if split_view then
        local sp=(split_view=='current') and active_split or find_split(tostring(split_view))
        return sp and ('Split '..sp.name) or 'Split'
    end
    if settings.period and settings.period~='session' then return Core.display_word(settings.period) end
    return 'Full Parse'
end

function Report.report_actors(source,show_all)
    local actors=display_actors(source,true)
    local limit=show_all and #actors or math.min(#actors,9)
...
function Report.report_denominator(source)
    local total=0
    for _,a in ipairs(display_actors(source,true)) do total=total+combined_damage(a) end
    return total
end

function Report.report_header(destination,tell_name,scope,source,count)
    local _,active=elapsed_active(source,Core.now())
    send_game_line(destination,('VP %s | %s | Parsing: %s | Active %s | Actors %d'):format(
        view_display_name(settings.view),Report.report_scope_label(),parsing_scope_text(),Core.duration(active),count or 0),tell_name)
end

function Report.send_percent_report(destination,tell_name,actors,source)
    local denom=Report.report_denominator(source)
    local share_sum=0
    for i,a in ipairs(actors) do
        local dmg=combined_damage(a); local _,ranged,ws,sc,magic,pet=Report.offense_values(a)
        local share=denom~=0 and 100*dmg/denom or 0; share_sum=share_sum+share
        local parts={
            ('#%d %s'):format(i,a.name),
            ('Tot Dmg%% %.1f%%'):format(share),
            ('WS %.1f%%'):format(denom~=0 and 100*ws/denom or 0),
            ('SC %.1f%%'):format(denom~=0 and 100*sc/denom or 0),
        }
        if math.abs(magic)>0 then parts[#parts+1]=('Magic %.1f%%'):format(denom~=0 and 100*magic/denom or 0) end
        if math.abs(ranged)>0 then parts[#parts+1]=('Ranged %.1f%%'):format(denom~=0 and 100*ranged/denom or 0) end
        if math.abs(pet)>0 then parts[#parts+1]=('Pet %.1f%%'):format(denom~=0 and 100*pet/denom or 0) end
        send_game_line(destination,table.concat(parts,' | '),tell_name)
    end
    local avg_share=#actors>0 and share_sum/#actors or 0
    send_game_line(destination,('Represented Share %.1f%% | Avg Share %.1f%%'):format(share_sum,avg_share),tell_name)
end

function Report.send_offense_report(destination,tell_name,actors,source,full)
    local denom=Report.report_denominator(source)
    local totals={dmg=0,melee=0,ranged=0,ws=0,sc=0,magic=0,pet=0,mb=0,crit=0,hits=0,attempts=0,ws_hits=0,ws_attempts=0,ws_damage=0,ws_count=0,sc_count=0,cured=0,taken=0}
    for i,a in ipairs(actors) do
        local dmg=combined_damage(a); local melee,ranged,ws,sc,magic,pet=Report.offense_values(a)
        local share=denom~=0 and 100*dmg/denom or 0
        local ws_share=denom~=0 and 100*ws/denom or 0
        local sc_share=denom~=0 and 100*sc/denom or 0
        local dpsv=combined_dps(a,source,Core.now())
        send_game_line(destination,('#%d %s | Tot Dmg%% %.1f%% | Tot Dmg %s | DPS %s | Melee %s | Acc %s'):format(
            i,a.name,share,n(dmg),dash(dpsv,n),n(melee),accuracy_text(a,false)),tell_name)
        send_game_line(destination,('WS %s (%.1f%%) | H/M %d/%d | WSAcc %s | WSAvg %s | SC# %d | SC %s (%.1f%%)'):format(
            n(ws),ws_share,tonumber(a.ws_hits) or 0,tonumber(a.ws_misses) or 0,
            pct(a.ws_hits,a.ws_attempts),dash(avg(a.ws_mm),n),tonumber(a.skillchain_count) or 0,n(sc),sc_share),tell_name)
        local extra={}
        if math.abs(ranged)>0 then extra[#extra+1]=('Ranged %s (%.1f%%) RAcc %s'):format(n(ranged),denom~=0 and 100*ranged/denom or 0,accuracy_text(a,true)) end
        if math.abs(magic)>0 then extra[#extra+1]=('Magic %s (%.1f%%)'):format(n(magic),denom~=0 and 100*magic/denom or 0) end
        if math.abs(pet)>0 then extra[#extra+1]=('Pet %s (%.1f%%)'):format(n(pet),denom~=0 and 100*pet/denom or 0) end
        local mb=enabled_filter('magic') and net(a.mb_damage,a.dheal_mb) or 0
        if math.abs(mb)>0 then extra[#extra+1]=('MB Dmg %s'):format(n(mb)) end
        local crit=tonumber(a.melee_crit) or 0
        if crit>0 and (tonumber(a.melee_hits) or 0)>0 then extra[#extra+1]=('Crit %s'):format(pct(crit,a.melee_hits)) end
        if full and ((tonumber(a.healing) or 0)>0 or (tonumber(a.taken) or 0)>0) then extra[#extra+1]=('Cured %s | Taken %s'):format(n(a.healing),n(a.taken)) end
        if #extra>0 then send_game_line(destination,table.concat(extra,' | '),tell_name) end
        totals.dmg=totals.dmg+dmg; totals.melee=totals.melee+melee; totals.ranged=totals.ranged+ranged; totals.ws=totals.ws+ws; totals.sc=totals.sc+sc; totals.magic=totals.magic+magic; totals.pet=totals.pet+pet
        totals.mb=totals.mb+(enabled_filter('magic') and net(a.mb_damage,a.dheal_mb) or 0); totals.crit=totals.crit+(tonumber(a.melee_crit) or 0)
        totals.hits=totals.hits+(tonumber(a.melee_hits) or 0); totals.attempts=totals.attempts+(tonumber(a.melee_attempts) or 0)
        totals.ws_hits=totals.ws_hits+(tonumber(a.ws_hits) or 0); totals.ws_attempts=totals.ws_attempts+(tonumber(a.ws_attempts) or 0)
        totals.ws_damage=totals.ws_damage+ws; totals.ws_count=totals.ws_count+(tonumber(a.ws_hits) or 0)
        totals.sc_count=totals.sc_count+(tonumber(a.skillchain_count) or 0); totals.cured=totals.cured+(tonumber(a.healing) or 0); totals.taken=totals.taken+(tonumber(a.taken) or 0)
    end
    local _,active=elapsed_active(source,Core.now())
    local combined_dps_value=active>0 and totals.dmg/active or nil
    local summary=('TOTAL | Dmg %s | DPS %s | Acc %s | WS %s | WSAcc %s | WSAvg %s | SC# %d | SC %s'):format(
        n(totals.dmg),dash(combined_dps_value,n),pct(totals.hits,totals.attempts),n(totals.ws),pct(totals.ws_hits,totals.ws_attempts),
        totals.ws_count>0 and n(totals.ws_damage/totals.ws_count) or '-',totals.sc_count,n(totals.sc))
    send_game_line(destination,summary,tell_name)
    local categories={('Melee %s'):format(n(totals.melee)),('WS %s'):format(n(totals.ws)),('SC %s'):format(n(totals.sc))}
    if math.abs(totals.ranged)>0 then categories[#categories+1]=('Ranged %s'):format(n(totals.ranged)) end
    if math.abs(totals.magic)>0 then categories[#categories+1]=('Magic %s'):format(n(totals.magic)) end
    if math.abs(totals.pet)>0 then categories[#categories+1]=('Pet %s'):format(n(totals.pet)) end
    if math.abs(totals.mb)>0 then categories[#categories+1]=('MB Dmg %s'):format(n(totals.mb)) end
    if totals.crit>0 and totals.hits>0 then categories[#categories+1]=('Crit %s'):format(pct(totals.crit,totals.hits)) end
    if full and totals.cured>0 then categories[#categories+1]=('Cured %s'):format(n(totals.cured)) end
    if full and totals.taken>0 then categories[#categories+1]=('Taken %s'):format(n(totals.taken)) end
    send_game_line(destination,'TOTAL DETAIL | '..table.concat(categories,' | '),tell_name)
end

function Report.send_magic_chat_report(destination,tell_name,actors,source)
    local total=0; for _,a in ipairs(actors) do local _,_,_,_,magic=Report.offense_values(a); total=total+magic end
    if total==0 then return false end
    for i,a in ipairs(actors) do
        local _,_,_,_,magic=Report.offense_values(a)
        if magic~=0 or (tonumber(a.magic_casts) or 0)>0 then
            send_game_line(destination,('#%d %s | Magic %s | Cast %d | Land/Res %d/%d | MB %d %s | Avg %s | Peak %s'):format(
                i,a.name,n(magic),tonumber(a.magic_casts) or 0,tonumber(a.magic_lands) or 0,tonumber(a.magic_resists) or 0,
                tonumber(a.mb_count) or 0,n(a.mb_damage or 0),dash(avg(a.magic_mm),n),dash(a.magic_mm and a.magic_mm.peak,n)),tell_name)
        end
    end
    send_game_line(destination,'TOTAL Magic '..n(total),tell_name)
    return true
end

function Report.send_cure_chat_report(destination,tell_name,actors,source)
    local cured,received=0,0
    for i,a in ipairs(actors) do
        if (tonumber(a.healing) or 0)>0 or (tonumber(a.received) or 0)>0 or (tonumber(a.cleanses) or 0)>0 then
            send_game_line(destination,('#%d %s | Cured %s | Cure Rcvd %s | Self %s | Cure# %d | Avg %s | Cleanse %d | Dispel %d'):format(
                i,a.name,n(a.healing),n(a.received),n(a.self_healing),tonumber(a.cures) or 0,dash(avg(a.cure_mm),n),tonumber(a.cleanses) or 0,tonumber(a.dispels) or 0),tell_name)
        end
        cured=cured+(tonumber(a.healing) or 0); received=received+(tonumber(a.received) or 0)
    end
    send_game_line(destination,('TOTAL | Cured %s | Cure Rcvd %s'):format(n(cured),n(received)),tell_name)
end

function Report.send_defense_chat_report(destination,tell_name,actors,source)
    local taken,hits,evades,parries,blocks,deaths=0,0,0,0,0,0
    for i,a in ipairs(actors) do
        send_game_line(destination,('#%d %s | Taken %s | Phys %s | Magic %s | Hits %d | AvgHit %s | Evd %d | Par %d | Blk %d | KO %d'):format(
            i,a.name,n(a.taken),n(a.taken_physical),n(a.taken_magical),tonumber(a.taken_hits) or 0,dash(avg(a.taken_mm),n),
            tonumber(a.evades) or 0,tonumber(a.parries) or 0,tonumber(a.blocks) or 0,tonumber(a.deaths) or 0),tell_name)
        taken=taken+(tonumber(a.taken) or 0); hits=hits+(tonumber(a.taken_hits) or 0); evades=evades+(tonumber(a.evades) or 0); parries=parries+(tonumber(a.parries) or 0); blocks=blocks+(tonumber(a.blocks) or 0); deaths=deaths+(tonumber(a.deaths) or 0)
    end
    send_game_line(destination,('TOTAL | Taken %s | Hits %d | AvgHit %s | Evd %d | Par %d | Blk %d | KO %d'):format(
        n(taken),hits,hits>0 and n(taken/hits) or '-',evades,parries,blocks,deaths),tell_name)
end

function Report.send_category_chat_report(destination,tell_name,actors,source,scope)
    if scope=='magic' then return Report.send_magic_chat_report(destination,tell_name,actors,source) end
    if scope=='cure' then return Report.send_cure_chat_report(destination,tell_name,actors,source) end
    if scope=='defense' then return Report.send_defense_chat_report(destination,tell_name,actors,source) end
    if scope=='percent' then return Report.send_percent_report(destination,tell_name,actors,source) end
    if scope=='full' or scope=='dps' then return Report.send_offense_report(destination,tell_name,actors,source,scope=='full') end

    -- Focused offensive category reports.
    local denom=Report.report_denominator(source); local sum=0
    for i,a in ipairs(actors) do
        local melee,ranged,ws,sc,magic,pet=Report.offense_values(a)
        local value=scope=='melee' and melee or scope=='ranged' and ranged or scope=='ws' and ws or scope=='sc' and sc or scope=='pet' and pet or 0
        sum=sum+value
        send_game_line(destination,('#%d %s | %s %s | %.1f%%'):format(i,a.name,Core.display_word(scope),n(value),denom~=0 and 100*value/denom or 0),tell_name)
    end
    send_game_line(destination,('TOTAL %s %s'):format(Core.display_word(scope),n(sum)),tell_name)
end

function Report.send_chat_report(destination,tell_name,scope,show_all)
    local source=selected_source(); if not source then chat(167,'No report data available.'); return end
    local actors=Report.report_actors(source,show_all)
    if scope=='magic' then
        local any=false; for _,a in ipairs(actors) do local _,_,_,_,m=Report.offense_values(a); if m~=0 or (tonumber(a.magic_casts) or 0)>0 then any=true break end end
        if not any then chat(167,'No tracked Magic data in the selected parse.'); return end
    end
    Report.report_header(destination,tell_name,scope,source,#actors)
    Report.send_category_chat_report(destination,tell_name,actors,source,scope or 'full')
    chat(207,('Queued %s report to %s%s (%d actors, %.2fs delay).'):format(
        scope or 'full',Report.DESTINATIONS[destination] or destination,tell_name and (' '..tell_name) or '',#actors,tonumber(settings.report_delay) or 0.65))
end

function Report.parse_chat_report_args(args)
    local destination,tell_name,scope,show_all=nil,nil,'full',false
    local i=2
    while i<=#args do
        local token=Core.lower(args[i] or '')
        if Report.DESTINATIONS[token] then
            destination=token
            if Report.DESTINATIONS[token]=='tell' then
                if args[i+1] then tell_name=args[i+1]; i=i+1 else return nil,nil,nil,nil,'Tell requires a player name.' end
            end
        elseif Report.SCOPES[token] then scope=Report.SCOPES[token]
        elseif token=='all' then show_all=true
        end
        i=i+1
    end
    if not destination then return nil,nil,nil,nil,nil end
    return destination,tell_name,scope,show_all,nil
end
...
local VP_COMMAND_SUBJECTS={
    hud={'on','off','start','stop','enable','disable'},
    parse={'on','off','start','stop','continue','pause','enable','disable','reset'},
    parser={'on','off','start','stop','continue','pause','enable','disable','reset'},
    attack={'on','off','start','stop','enable','disable','include','exclude','report'},
    melee={'on','off','start','stop','enable','disable','include','exclude','report'},
    ranged={'on','off','start','stop','enable','disable','include','exclude','report'},
    ws={'on','off','start','stop','enable','disable','include','exclude','report'},
    sc={'on','off','start','stop','enable','disable','include','exclude','report'},
    magic={'on','off','start','stop','enable','disable','include','exclude','report'},
    pet={'on','off','start','stop','enable','disable','include','exclude','report'},
    other={'on','off','start','stop','enable','disable','include','exclude'},
    healing={'report'}, cure={'report'}, damage={'report'},
    trusts={'on','off','start','stop','enable','disable','include','exclude'},
    allies={'on','off','start','stop','enable','disable','include','exclude'},
    target={'on','off','start','stop','enable','disable'},
    highlights={'on','off','start','stop','enable','disable'},
    log={'on','off','start','stop','enable','disable'},
}
local function vp_normalize_subject_command(args)
    local subject,verb=Core.command_subject_verb(args,VP_COMMAND_SUBJECTS)
    if not subject then return args end
    local canonical=({attack='melee',cure='healing',parser='parse'})[subject] or subject
    local positive=(verb=='on' or verb=='start' or verb=='enable' or verb=='include')
    local negative=(verb=='off' or verb=='stop' or verb=='disable' or verb=='exclude')
    if canonical=='hud' then return {positive and 'show' or negative and 'hide' or verb}
    elseif canonical=='parse' then
        if verb=='reset' then return {'reset','session'} end
        if verb=='continue' or positive then return {'continue'} end
        if verb=='pause' or negative then return {'pause'} end
    elseif canonical=='trusts' or canonical=='allies' or canonical=='target' or canonical=='highlights' then
        if positive then return {canonical,'on'} elseif negative then return {canonical,'off'} end
    elseif canonical=='log' then
        if positive then return {'log','on'} elseif negative then return {'log','off'} end
    elseif verb=='report' then return {'report',canonical}
    elseif positive then return {'include',canonical}
    elseif negative then return {'exclude',canonical} end
    return args
end

local function help_line(text) chat(207,Core.humanize_command_text(text)) end
local function print_help()
    help_line('Universal vocabulary (where applicable): '..Core.command_options(Core.COMMAND_VERBS))
    help_line('//vp dynamic | general | compact | physical | ranged | magic [overall | full] | defense | combo | healing | pet | ws [overall | full]')
    help_line('//vp period session | current | last | scope self | party | alliance | custom | sort damage | dps | party | healing | taken | wsavg')
    help_line('//vp add | remove ranged | crits | pet | healing | pet damage type | ws count | //vp columns')
    help_line('//vp include | exclude melee | ranged | ws | sc | magic | pet | petmelee | petranged | petphysical | petmagic | petsc | other | all')
    help_line('Subject-first forms also work where applicable: attack on | ws report | ws enable | ws exclude | trusts off | parser pause')
    help_line('//vp pin me | <player> [slot] | unpin me | <player> | all | pins')
    help_line('//vp target on | off | auto | rows <1-18 | all | reset> | trusts on | off | allies on | off')
    help_line('//vp report [full | dps | magic | cure | defense | % | ws | melee | ranged | sc | pet] <party | alliance | linkshell | linkshell2 | say | yell | tell <name>> [all]')
    help_line('//vp reportdelay <seconds> | highlights on | off | toggle | log on | off | status')
    help_line('//vp filter add | remove <enemy | current | damage type> | filter <damage type> | filter list | clear | //vp split [name] | splits | unsplit | reset | clear | reload')
end
...
local function handle_filter_command(args)
    local op=Core.lower(args[2] or 'list')
    if op=='clear' or op=='reset' or op=='all' then settings.enemy_filter_text=''; settings.enemy_filter_ids=''; all_damage_filters(true); chat(207,'Filters cleared.'); return end
    if op=='list' or op=='status' then chat(207,filter_status_text()); return end
    local direct=FILTER_ALIASES[op] or FILTER_ALIASES[op:gsub('%s+','')]
    if direct then all_damage_filters(false); settings.filters[direct]=true; chat(207,'Filter only: '..direct..'.'); return end
    if op=='add' or op=='include' or op=='remove' or op=='exclude' then
        local value=Core.join({select(3,unpack(args))},' '); local token=Core.lower(value); local canonical=FILTER_ALIASES[token] or FILTER_ALIASES[token:gsub('%s+','')]
        if canonical then settings.filters[canonical]=(op=='add' or op=='include'); chat(207,(settings.filters[canonical] and 'Included ' or 'Excluded ')..canonical..'.'); return end
        if token=='current' then
            if op=='add' or op=='include' then add_current_target_filter() else settings.enemy_filter_ids='' end
            return
        end
        if value~='' then if op=='add' or op=='include' then add_enemy_filter_text(value); chat(207,'Enemy filter added: '..value) else remove_enemy_filter_text(value); chat(207,'Enemy filter removed: '..value) end; return end
    end
    -- A bare non-category term behaves as an enemy filter for convenience.
    local value=Core.join({select(2,unpack(args))},' '); if value~='' then add_enemy_filter_text(value); chat(207,'Enemy filter added: '..value) else chat(167,'Use //vp filter add <enemy | current | damage type>.') end
end
...
windower.register_event('addon command',function(...)
    local args=vp_normalize_subject_command({...}); local cmd=Core.lower(args[1] or 'help')
    if cmd=='help' or cmd=='?' then print_help(); return
    elseif cmd=='show' then settings.visible=true; hud:show()
    elseif cmd=='hide' then settings.visible=false; hud:hide()
    elseif cmd=='toggle' then settings.visible=not settings.visible; if settings.visible then hud:show() else hud:hide() end
    elseif cmd=='compact' then settings.view='compact'
    elseif cmd=='full' or cmd=='dynamic' or cmd=='general' or cmd=='overview' then settings.view='dynamic'
    elseif cmd=='physical' then settings.view='physical'
    elseif cmd=='combo' then settings.view='combo'
    elseif cmd=='rows' then
        local value=Core.lower(args[2] or 'reset'); if value=='all' or value=='off' then settings.row_limit=0 elseif value=='reset' or value=='default' then settings.row_limit=8 else local nrows=tonumber(args[2]); if nrows then settings.row_limit=Core.clamp(math.floor(nrows),1,18) else chat(167,'Use //vp rows <1-18 | all | reset>.') end end
        chat(207,'Visible actor rows: '..(settings.row_limit==0 and 'All' or tostring(settings.row_limit)))
    elseif cmd=='filter' then handle_filter_command(args)
    elseif cmd=='split' then handle_split_command(args)
    elseif cmd=='splits' then handle_split_command({'split','list'})
    elseif cmd=='unsplit' then split_view=nil; chat(207,'Showing full session.')
    elseif cmd=='highlights' or cmd=='highlight' then
        local mode=Core.lower(args[2] or 'toggle')
        if mode=='on' or mode=='enable' or mode=='enabled' then settings.highlights=true
        elseif mode=='off' or mode=='disable' or mode=='disabled' then settings.highlights=false
        elseif mode=='toggle' then settings.highlights=not (settings.highlights~=false)
        else chat(167,'Highlights: on | off | toggle') end
        chat(207,'Leader highlights '..(settings.highlights~=false and 'On' or 'Off')..'.')
    elseif (cmd=='enable' or cmd=='disable') and (Core.lower(args[2] or '')=='highlights' or Core.lower(args[2] or '')=='highlight') then
        settings.highlights=(cmd=='enable'); chat(207,'Leader highlights '..(settings.highlights and 'On' or 'Off')..'.')
    elseif cmd=='pause' or cmd=='stop' then settings.paused=true; chat(207,'Paused.')
    elseif cmd=='resume' or cmd=='continue' or cmd=='start' then settings.paused=false; chat(207,'Active.')
    ...
    elseif cmd=='pos' then local x,y=tonumber(args[2]),tonumber(args[3]); if x and y then hud:pos(x,y) end
    elseif cmd=='size' then local size=Core.clamp(tonumber(args[2]) or 8,5,36); hud:size(size); settings.hud.text.size=size
    elseif cmd=='add' or cmd=='remove' then
        local on=cmd=='add'; local what=Core.lower(args[2] or '')
        local combined=Core.lower(Core.join({select(2,unpack(args))},' '))
        if what=='ranged' or what=='range' then set_column('ranged',on and true or 'auto')
        elseif what=='crits' or what=='crit' then set_column('crits',on)
        elseif what=='pet' then set_column('pet',on and true or 'auto')
        elseif what=='healing' or what=='heal' then set_column('healing',on and true or 'auto')
        elseif combined=='pet damage type' or combined=='pdmg type' or combined=='pet type' then set_column('pet_types',on); if on then set_column('pet',true) end
        else chat(167,'Columns: ranged | crits | pet | healing | pet damage type | ws count') end
    elseif cmd=='columns' then
        chat(207,('Columns | Ranged %s | Crits %s | Pet %s | Pet Types %s | Healing %s | WS Count %s'):format(tostring(settings.columns.ranged),tostring(settings.columns.crits),tostring(settings.columns.pet),tostring(settings.columns.pet_types),tostring(settings.columns.healing),tostring(settings.columns.ws_count)))
    elseif cmd=='include' or cmd=='exclude' then
        local include=cmd=='include'; local token=Core.lower(args[2] or '')
        if token=='all' then for k in pairs(defaults.filters) do settings.filters[k]=include end
        else
            local canonical=FILTER_ALIASES[token] or FILTER_ALIASES[token:gsub('%s+','')]
            if canonical then settings.filters[canonical]=include else chat(167,'Unknown damage filter.') end
        end
    elseif cmd=='pin' then
        local name=args[2]; if not name then chat(167,'Use //vp pin me | <player> [slot].') else
            local p=windower.ffxi.get_player(); if Core.lower(name)=='me' then name=p and p.name or name end
            local slot=tonumber(args[3]); slot=slot and math.max(1,math.floor(slot)) or nil
            if slot then
                -- A newly assigned fixed slot takes precedence. Shift an
                -- existing fixed occupant downward, cascading as necessary.
                local occupied=slot
                while occupied do
                    local displaced=nil
                    for other,other_slot in pairs(settings.pins) do if other~=Core.lower(name) and tonumber(other_slot)==occupied then displaced=other; break end end
                    if displaced then settings.pins[displaced]=occupied+1; occupied=occupied+1 else occupied=nil end
                end
            end
            settings.pins[Core.lower(name)]=slot or true
            chat(207,('Pinned %s%s.'):format(name,slot and (' at '..slot) or ''))
        end
    elseif cmd=='unpin' then
        local name=args[2]; if Core.lower(name)=='all' then settings.pins={}
        elseif name then local p=windower.ffxi.get_player(); if Core.lower(name)=='me' then name=p and p.name or name end; settings.pins[Core.lower(name)]=nil end
    elseif cmd=='pins' then
        local list={}; for name,slot in pairs(settings.pins or {}) do list[#list+1]=name..(tonumber(slot) and ('@'..slot) or '') end; table.sort(list)
        chat(207,'Pins: '..(#list>0 and table.concat(list,', ') or 'None')..' | Self soft-pin '..(settings.self_pin~=false and 'On' or 'Off'))
    elseif cmd=='reportdelay' or cmd=='report-delay' then
        settings.report_delay=Core.clamp(tonumber(args[2]) or 0.65,0.10,3.00); chat(207,('Report delay %.2fs.'):format(settings.report_delay))
    elseif cmd=='trusts' or cmd=='trust' then
        local mode=Core.lower(args[2] or 'toggle'); if mode=='on' then settings.include_trusts=true elseif mode=='off' then settings.include_trusts=false else settings.include_trusts=not (settings.include_trusts~=false) end; chat(207,'Trust rows '..(settings.include_trusts~=false and 'On' or 'Off')..'.')
    elseif cmd=='allies' or cmd=='allied' then
        local mode=Core.lower(args[2] or 'toggle'); if mode=='on' then settings.include_allied_npcs=true elseif mode=='off' then settings.include_allied_npcs=false else settings.include_allied_npcs=not (settings.include_allied_npcs~=false) end; chat(207,'Fellow/allied NPC rows '..(settings.include_allied_npcs~=false and 'On' or 'Off')..'.')
    elseif cmd=='target' then
        local mode=Core.lower(args[2] or 'auto'); if mode=='on' or mode=='off' or mode=='auto' then settings.target_hp=mode else chat(167,'Target: on | off | auto') end
    elseif cmd=='scope' then if not set_scope(args[2]) then chat(167,'Scope: self | party | alliance | custom') end
    elseif cmd=='players' then settings.custom_players={}; for i=2,#args do settings.custom_players[#settings.custom_players+1]=args[i] end; settings.scope='custom'
    elseif cmd=='player' then
        local sub=Core.lower(args[2]); local name=args[3]
        if sub=='clear' then settings.custom_players={}; settings.scope='custom'
        elseif name and sub=='add' then local exists=false; for _,v in ipairs(settings.custom_players) do if v:lower()==name:lower() then exists=true end end; if not exists then settings.custom_players[#settings.custom_players+1]=name end; settings.scope='custom'
        elseif name and sub=='remove' then for i=#settings.custom_players,1,-1 do if settings.custom_players[i]:lower()==name:lower() then table.remove(settings.custom_players,i) end end
        elseif args[2] then settings.custom_players={args[2]}; settings.scope='custom' end
    elseif cmd=='period' then local p=Core.lower(args[2]); if p=='current' or p=='last' or p=='session' then settings.period=p end
    elseif cmd=='sort' then local s=Core.lower(args[2]); local valid={party=true,damage=true,dps=true,melee=true,accuracy=true,acc=true,ranged=true,racc=true,ws=true,wsacc=true,wsavg=true,sc=true,magic=true,pet=true,healing=true,cleanse=true,cleanses=true,dispel=true,dispels=true,taken=true}; if valid[s] then settings.sort=s else chat(167,'Sort: party | damage | dps | melee | acc | ranged | racc | ws | wsacc | wsavg | sc | magic | pet | healing | cleanse | dispel | taken') end
    elseif cmd=='enemy' then
        local value=Core.join({select(2,unpack(args))},' '); settings.enemy_filter_text=''; settings.enemy_filter_ids=''; if Core.lower(value)=='all' or value=='' then chat(207,'Enemy filter cleared.') elseif Core.lower(value)=='current' then add_current_target_filter() else add_enemy_filter_text(value); chat(207,'Enemy filter: '..value) end
    elseif cmd=='ws' then settings.view=(Core.lower(args[2])=='full') and 'ws-full' or 'ws-overall'
    elseif cmd=='magic' then settings.view=(Core.lower(args[2])=='full') and 'magic-full' or 'magic-overall'
    elseif cmd=='max' then apply_max_command(args)
    elseif cmd=='melee' then settings.view='physical'
    elseif cmd=='ranged' or cmd=='range' then settings.view='ranged'
    elseif cmd=='defense' or cmd=='healing' or cmd=='pet' then set_view(cmd)
    elseif cmd=='report' then
        local destination,tell_name,scope,show_all,report_error=Report.parse_chat_report_args(args)
        if report_error then chat(167,report_error)
        elseif destination then Report.send_chat_report(destination,tell_name,scope,show_all)
        else
            local report_type=Core.lower(args[2] or 'full')
            if report_type=='damage' or report_type=='dmg' or report_type=='dps' or report_type=='full' then print_report(selected_source())
            elseif report_type=='enemy' and args[3] then print_report(selected_source(),Core.join({select(3,unpack(args))},' '))
            elseif report_type=='ws' or report_type=='weaponskill' or report_type=='weaponskills' then print_ws_report(selected_source())
            elseif report_type=='melee' or report_type=='attack' or report_type=='ranged' or report_type=='sc' or report_type=='skillchain' or report_type=='pet' or report_type=='other' then print_category_report(selected_source(),report_type=='attack' and 'melee' or report_type=='skillchain' and 'sc' or report_type)
            elseif report_type=='magic' or report_type=='spell' then local name=#args>=3 and Core.join({select(3,unpack(args))},' ') or nil; print_magic_report(selected_source(),name)
            elseif report_type=='dheal' or report_type=='damageheal' then print_dheal_report(selected_source())
            elseif report_type=='healing' or report_type=='cure' or report_type=='cured' then print_healing_report(selected_source())
            elseif report_type=='defense' or report_type=='def' then chat(207,'Use //vp report defense <party|alliance|linkshell|say|yell|tell name> for paced chat output.')
            elseif report_type=='percent' or report_type=='percentage' or report_type=='%' then chat(207,'Use //vp report % <party|alliance|linkshell|say|yell|tell name>.')
            else print_report(selected_source()) end
        end
    elseif cmd=='log' then
        local state=Core.lower(args[2] or 'status')
        if state=='on' or state=='enable' or state=='start' then settings.log_enabled=true
        elseif state=='off' or state=='disable' or state=='stop' then settings.log_enabled=false
        elseif state~='status' then chat(167,Core.humanize_command_text('Use //vp log on | off | status.')) end
        chat(207,'Persistent logging: '..(settings.log_enabled and 'On' or 'Off'))
    elseif cmd=='save' then
        settings.hud=hud:settings(); config.save(settings); enemy_registry:save(); chat(207,'Settings and learned registry saved.')
    elseif cmd=='reload' then
        settings.hud=hud:settings(); config.save(settings); enemy_registry:save(); windower.send_command('lua reload VanaParse'); return
    elseif cmd=='unload' then
        settings.hud=hud:settings(); config.save(settings); enemy_registry:save(); windower.send_command('lua unload VanaParse'); return
    elseif cmd=='logs' then
        ensure_log_dir(); chat(207,'Persistent TSV log: '..log_file)
    elseif cmd=='history' then
        local count=Core.clamp(math.floor(tonumber(args[2]) or 5),1,settings.history_limit)
        if #history==0 then chat(207,'No completed encounters in memory.') else
            local first=math.max(1,#history-count+1)
            for i=first,#history do local h=history[i]; local total=0; for _,a in pairs(h.actors or {}) do if a.actor_type~='enemy' and a.actor_type~='pet' and a.session_scope~='outside' then total=total+combined_damage(a) end end; local elapsed=math.max(0,(h.ended or Core.now())-(h.started or Core.now())); chat(207,('%d. %s | %s | Party/Alliance dmg %s'):format(i,h.id or 'Fight',Core.duration(elapsed),Core.compact(total))) end
        end
    elseif cmd=='reset' or cmd=='clear' then
        local what=Core.lower(args[2] or 'session'); if what=='fight' or what=='encounter' then current=nil; forced_miss_windows={}; chat(207,'Current encounter cleared.') else full_reset('manual-reset'); chat(207,'Full parse session cleared.') end
    elseif cmd=='registry' then
        local sub=Core.lower(args[2] or 'status')
        if sub=='status' then local st=enemy_registry:status(); chat(207,('Enemy Registry %s | bundled %d | external %d | learned %d | user %d'):format(st.version,st.bundled,st.external,st.learned,st.user))
        elseif sub=='save' then local ok=enemy_registry:save(); chat(ok and 207 or 167,ok and 'Enemy registry saved.' or 'Enemy registry save failed.')
        elseif sub=='reload' then enemy_registry:save(); enemy_registry=new_enemy_registry(); local st=enemy_registry:status(); chat(207,('Enemy registry reloaded | %s | bundled %d | external %d | learned %d | user %d'):format(st.version,st.bundled,st.external,st.learned,st.user))
        else chat(167,'Use //vp registry status | save | reload.') end
    elseif cmd=='health' or cmd=='debug' then
        local actors=0; if current then for _ in pairs(current.actors) do actors=actors+1 end end
        chat(207,('Version %s | Core %s | Events %d | Current actors %d | History %d/%d | Errors %d | Raw retained 0'):format(_addon.version,Core.VERSION,event_count,actors,#history,settings.history_limit,errors)); if last_error then chat(167,'Last error: '..last_error) end
    else print_help() end
